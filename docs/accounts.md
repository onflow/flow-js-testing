...
However, under some circumstances the user may wish to create accounts (see: [`createAccount`](#createaccountname-keys)) or sign for accounts (see: [`sendTransaction`](./send-transactions.md)) using custom private keys (i.e. private key value, [hashing algorithm](./send-transactions.md#hashalgorithm), [signing algorithm](./send-transactions.md#signaturealgorithm), etc.) - this functionality is facilitated by the aforementioned methods.
...
| `keys` | [[KeyObject](#keyobject)] | No       | An array of [KeyObjects](#keyobject) to be added to the account upon creation (default: [universal private key](#universal-private-key))   |
